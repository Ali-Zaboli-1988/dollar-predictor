...
class TestValidatePriceHistory(unittest.TestCase):
    ...
    def test_valid_series(self):
        path = self.write_csv([["2026-06-01", "100000"], ["2026-06-02", "101000"], ["2026-06-03", "100500"]])
        result = validate(path, minimum_rows=3)
        self.assertEqual(result.rows, 3)

    def test_duplicate_date_is_rejected(self):
        path = self.write_csv([["2026-06-01", "100000"], ["2026-06-01", "101000"], ["2026-06-03", "100500"]])
        with self.assertRaises(ValueError):
            validate(path, minimum_rows=3)

    def test_unsorted_dates_are_rejected(self):
        path = self.write_csv([["2026-06-02", "100000"], ["2026-06-01", "101000"], ["2026-06-03", "100500"]])
        with self.assertRaises(ValueError):
            validate(path, minimum_rows=3)

    def test_large_jump_is_rejected(self):
        path = self.write_csv([["2026-06-01", "100000"], ["2026-06-02", "130000"], ["2026-06-03", "131000"]])
        with self.assertRaises(ValueError):
            validate(path, minimum_rows=3, max_daily_change_pct=25.0)

    def test_non_positive_price_is_rejected(self):
        path = self.write_csv([["2026-06-01", "100000"], ["2026-06-02", "0"], ["2026-06-03", "101000"]])
        with self.assertRaises(ValueError):
            validate(path, minimum_rows=3)
